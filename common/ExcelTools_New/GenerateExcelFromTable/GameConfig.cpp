...
void GameConfig::ParseAll()
{
	...
	//读取配置文件
	{
		...
		for (rapidxml::xml_node<char> *pNode = root->first_node(); pNode != NULL; pNode = pNode->next_sibling())
		{
			...
			else if(!strcmp(pNode->name(), "GenerateExcelFromTable"))
			{
				for (rapidxml::xml_attribute<>* attr = pNode->first_attribute(); attr; attr = attr->next_attribute())
				{
					m_GenerateExcelFromTableConfig.m_svnDir = attr->value();
					break;
				}
				for (xml_node<char>* pChildNode = pNode->first_node(); pChildNode != NULL; pChildNode = pChildNode->next_sibling())
				{
					m_GenerateExcelFromTableConfig.m_GenerateExcelFromTableConfig.push_back(pChildNode->value());
				}
			}
		}
	}
...
const GenerateExcelFromTableConfig& GameConfig::GetGenerateExcelFromTableConfig()
{
	return m_GenerateExcelFromTableConfig;
}