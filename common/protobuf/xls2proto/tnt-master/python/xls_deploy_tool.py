...
##
FIELD_COMMENT_ROW = 1   #FIELD_COMMENT_ROW = 3
FIELD_RULE_ROW = 2      #FIELD_RULE_ROW = 0
FIELD_NAME_ROW = 3      #FIELD_NAME_ROW = 2
# 这一行还表示重复的最大个数，或结构体元素数
FIELD_TYPE_ROW = 4      #FIELD_TYPE_ROW = 1
...
class SheetInterpreter:
    ...
    def __init__(self, xls_file_path, sheet_name):
        ...
        real_sheet_name = ""
        for i in range(len(sheet_name.split("_"))):
            if (0 == i):
                continue
            real_sheet_name += sheet_name.split("_")[i] + "_"
        
        #self._sheet_name = sheet_name.split("_")[1].encode('utf-8')
        #self._sheet_name = sheet_name.encode('utf-8')
        self._sheet_name = real_sheet_name[:-1].encode('utf-8')
        ...
        try :
            ...
            print "open sheet(%s) failed!"%(sheet_name)
            print e
        ...
        self._pb_file_name = "./proto/" + self._sheet_name.lower() + ".proto"
    ...
    def Interpreter(self) :
        ...
        #self._LayoutStructHead(self._sheet_name.split("_")[1].encode('utf-8'))
    ...
    def _FieldDefine(self, repeated_num) :
        ...
        if field_rule == "required" or field_rule == "optional" :
            ...
            if (self._IsStructDefined(struct_name)) :
                self._is_layout = Falsesheet_name
            else :
...
def func(sheet_name, xls_file_path, op):

    '''添加代码符合策划需要的情形'''
    if (sheet_name.find("_") < 0):
        print "sheet name shoule be Chinese with English"
        sys.exit(-2)
    
    real_sheet_name = ""
    for i in range(len(sheet_name.split("_"))):
        #print 'sheet_name.split("_")[i]: ', sheet_name.split("_")[i], '\n'
        if (0 == i):
            continue
        real_sheet_name += sheet_name.split("_")[i] + "_"

    #print "real_sheet_name: ", real_sheet_name[:-1]
    
    #if (not sheet_name.isupper()):
    if (not real_sheet_name[:-1].isupper()):
        print "sheet_name should be upper"
        sys.exit(-2)

    #xls_file_path =  sys.argv[2]
    print xls_file_path, '\n'
    #print "type(xls_file_path): ", type(xls_file_path), '\n'
    #print "type(xls_file_path): ", type(xls_file_path.decode("utf-8")), '\n'
    #print "type(xls_file_path): ", type(xls_file_path.decode("utf-8").encode("gbk")), '\n'
    #print "type(xls_file_path): ", type(xls_file_path.decode("utf-8").encode("unicode")), '\n'
    
    if op == 0 or op == 1:
        try :
            #tool = SheetInterpreter(xls_file_path.decode('utf-8'), sheet_name.decode('utf-8'))
...
if __name__ == '__main__' :
    ...
    #sheet_name =  sys.argv[1].decode('utf-8')
    ...
    xls_file_path =  sys.argv[2]
    #xls_file_path = sys.argv[2].decode('gbk').encode('utf-8')
    #xls_file_path = sys.argv[2].decode('utf-8')
